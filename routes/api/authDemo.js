...
const TaskDemo = require('../../models/TaskDemo')
...
router.post('/resId', async (req, res)=> {
	...
	try {
		const user = await UserDemo.findOne({reqRes: req.body.id});
		res.json(user)
...
router.post('/updateAccount', auth, async (req, res)=> {
	...
	try{
		if(method === 'minus'){
			...
			const task = await TaskDemo.findOne({_id: req.body.taskId})
			const userMinus = await UserDemo.findById({_id: task.user});
			let newAccountBalance = (userMinus.account - req.body.amount)
			updateAccount = await UserDemo.findOneAndUpdate(
													{_id: userMinus._id},
													{account: newAccountBalance}
												)
			
			userUpdated = await UserDemo.findOne({_id: userMinus._id});
	
		} else if (method === 'Gift'){
			// Using taskId as userId
			const user = await UserDemo.findOne({_id: req.body.id});
			const filter = {_id: req.body.id};
			const update = {account: user.account + parseInt(req.body.amount)};
			updateAccount = await UserDemo.findOneAndUpdate(filter, update);
			userUpdated = await UserDemo.findOne({_id: req.body.id});
			...
			const user = await UserDemo.findOne({_id: req.user.id});
			const filter = {_id: req.user.id};
			const update = {account: user.account + updateValue};
			updateAccount = await UserDemo.findOneAndUpdate(filter, update);
			userUpdated = await UserDemo.findOne({_id: req.user.id});
		}
...
router.get('/getAccount', auth, async (req, res)=> {
	try {
		const user = await UserDemo.findById(req.user.id).select('-password');
...
async (req, res)=> {
	...
	try {
		// See if user exists
		let user = await UserDemo.findOne({email});
		if(!user){
			return	res
				.status(400)
...
router.post("/forgot", async (req, res) => {
	try {
		const thisUser = await UserDemo.findOne({email: req.body.email});
		if(thisUser) {
			const id = uuidv4();
			const request = {
				id,
