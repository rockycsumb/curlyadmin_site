...
export const getTasks = () => async dispatch => {
	...
	try {
		const res = await axios.get(`${apiURL}api/demo/task`);
		dispatch({
			type: GET_TASKS,
			payload: res.data
		})
...
export const getTaskById = id => async dispatch => {
	try {
		const res = await axios.get(`${apiURL}api/demo/task/${id}`);
		dispatch({
			type: GET_TASK,
			payload: res.data
		})
...
export const deleteTask = id => async dispatch => {
	try {
		const res = await axios.delete(`${apiURL}api/demo/task/${id}`);
		dispatch({
			type: DELETE_TASK,
			payload: {id}
		})
...
export const addTask = (formData, history, edit = false) => async dispatch => {
	try {
		...
		const res = await axios.post(`${apiURL}api/demo/task`, formData, config);
		dispatch({
			type: ADD_TASK,
			payload: res.data
		})
		...
		if(!edit){
			history.push('/demo/dashboard/task')
		} else {
			history.push('/demo/dashboard/task')
		}
	} catch(err) {
...
export const editTask = (formData, history, edit = true, taskId) => async dispatch => {
	try {
		...
		let res = await axios.patch(`${apiURL}api/demo/task/${taskId}`, formData, config);
...
export const addComment = (taskId, formData) => async dispatch => {
	try {
		...
		const res = await axios.post(`${apiURL}api/demo/task/comment/${taskId}`, formData, config);
		dispatch({
			type: ADD_COMMENT,
			payload: res.data
		})
...
export const deleteComment = (taskId, commentId) => async dispatch => {
	try {
		const res = await axios.delete(`${apiURL}api/demo/task/comment/${taskId}/${commentId}`);
		dispatch({
			type: DELETE_COMMENT,
			payload: commentId
		})
